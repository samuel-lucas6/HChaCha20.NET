using System;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using HChaCha20DotNet;

namespace HChaCha20DotNetTests;

[TestClass]
public class HChaCha20Tests
{
    // https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-xchacha#section-2.2.1
    [TestMethod]
    public void InternetDraft()
    {
        Span<byte> expected = stackalloc byte[]
        {
            0x82, 0x41, 0x3b, 0x42, 0x27, 0xb2, 0x7b, 0xfe,
            0xd3, 0x0e, 0x42, 0x50, 0x8a, 0x87, 0x7d, 0x73,
            0xa0, 0xf9, 0xe4, 0xd5, 0x8a, 0x74, 0xa8, 0x53,
            0xc1, 0x2e, 0xc4, 0x13, 0x26, 0xd3, 0xec, 0xdc
        };
        Span<byte> key = stackalloc byte[]
        {
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
            0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
            0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f
        };
        Span<byte> nonce = stackalloc byte[]
        {
            0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x4a,
            0x00, 0x00, 0x00, 0x00, 0x31, 0x41, 0x59, 0x27
        };
        Span<byte> outputKeyingMaterial = stackalloc byte[HChaCha20.OutputSize];
        HChaCha20.DeriveKey(outputKeyingMaterial, key, nonce);
        Assert.IsTrue(outputKeyingMaterial.SequenceEqual(expected));
    }
}